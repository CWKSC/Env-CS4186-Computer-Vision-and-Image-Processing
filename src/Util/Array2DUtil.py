...
def iterEle(array2d: list[list[int]]) -> Generator:
...
def centerAlignIter(array2d: list[list[int]]) -> Generator:
    k = getMinRadius(array2d)
    return ((r - k, c - k) for (r, c) in iter(array2d))


def centerAlignArray2D(k: int) -> Generator:
    return ((x - k, y - k) for y in range(2 * k + 1) for x in range(2 * k + 1))
